...
greybodyparams::greybodyparams(double r, double M_2, double M_3){
  alpha_ = get_alpha(r);
  x_ = x_param(alpha, M_2, M_3);
  q0_ = q0(alpha, M_2, M_3);
}

int main(){
  greybodyparams test{1.6, 2, 2};
  greybodyparams test = greybodyparams::greybodyparams(1.6, 2, 2);
  std::cout << test.alpha_ << test.x_ << std::endl;
  return 0;
}
